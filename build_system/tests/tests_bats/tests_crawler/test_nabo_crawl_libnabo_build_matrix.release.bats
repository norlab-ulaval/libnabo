...
TESTED_FILE="nabo_crawl_libnabo_build_matrix.release.bash"
TESTED_FILE_PATH="./build_system/"
COMPOSE_FILE="docker-compose.libnabo.yaml"
DOTENV_BUILD_MATRIX="${SRC_CODE_PATH}"/build_system/.env.build_matrix.libnabo.release
...
@test "${TESTED_FILE} › docker image › execute ok › expect pass" {
  ...
  run bash "${TESTED_FILE}" "${DOTENV_BUILD_MATRIX}" --fail-fast -- build --push --dry-run release
