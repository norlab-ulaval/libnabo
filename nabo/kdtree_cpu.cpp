...
namespace Nabo
{
	...
	template<typename T>
	size_t ANNKDTree<T>::getTreeSize(size_t elCount) const
	{
		// FIXME: 64 bits safe stuff, only work for 2^32 elements right now
		size_t count = 0;
		int i = 31;
	...
	template<typename T>
	size_t ANNKDTree<T>::argMax(const Vector& v) const
	{
		T maxVal(0);
		size_t maxIdx(0);
		for (size_t i = 0; i < this->dim; ++i)
	...
	template<typename T>
	typename ANNKDTree<T>::IndexVector ANNKDTree<T>::cloudIndexesFromNodesIndexes(const IndexVector& indexes) const
	{
		IndexVector cloudIndexes(indexes.size());
		for (size_t i = 0; i < indexes.size(); ++i)
			cloudIndexes.coeffRef(i) = nodes[indexes[i]].index;
	...
	template<typename T>
	void ANNKDTree<T>::buildNodes(const BuildPointsIt first, const BuildPointsIt last, const size_t pos)
	{
		const size_t count(last - first);
		//cerr << count << endl;
		if (count == 1)
	...
	template<typename T>
	void ANNKDTree<T>::dump(const Vector minValues, const Vector maxValues, const size_t pos) const
	{
		const Node& node(nodes[pos]);
	...
	template<typename T>
	ANNKDTree<T>::ANNKDTree(const Matrix& cloud):
		NearestNeighborSearch<T>::NearestNeighborSearch(cloud)
	{
		// build point vector and compute bounds
		vector<BuildPoint> buildPoints;
	...
	template<typename T>
	typename ANNKDTree<T>::IndexVector ANNKDTree<T>::knn(const Vector& query, const Index k, const unsigned optionFlags)
	{
		const bool allowSelfMatch(optionFlags & NearestNeighborSearch<T>::ALLOW_SELF_MATCH);
	...
	template<typename T>
	void ANNKDTree<T>::recurseKnn(const Vector& query, const size_t n, T rd, Heap& heap, Vector& off, const bool allowSelfMatch)
	{
		const Node& node(nodes[n]);
	...
	template struct ANNKDTree<float>;
	template struct ANNKDTree<double>;
}
